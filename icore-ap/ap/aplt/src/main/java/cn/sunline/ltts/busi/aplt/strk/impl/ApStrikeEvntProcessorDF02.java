...
public class ApStrikeEvntProcessorDF02 implements ApStrikeEvntProcessor {
	private static final BizLog bizlog = BizLogUtil.getBizLog(ApStrikeEvntProcessorDF02.class);

	@Override
	public void process(E_STACPS stacps, E_COLOUR colour, KnbEvnt evnt) {
		if (bizlog.isDebugEnabled())
			bizlog.debug("险退保冲正处理开始=====================");
		if (CommUtil.compare(evnt.getTranev(), ApUtil.TRANS_EVENT_DFBKIU) != 0)
			throw ApError.Aplt.E0000("交易事件不是[" + ApUtil.TRANS_EVENT_DFBKIU + "]，不能冲正！");
		...
		input.setBakup1(evnt.getEvent3()); // 保单号

		SysUtil.getInstance(IoStrikeSvcType.class).prcInsuBackStrike(input);

		if (bizlog.isDebugEnabled())
			bizlog.debug("险退保冲正处理结束=====================");
	}
