...
public class ApStrikeEvntProcessorCD03 implements ApStrikeEvntProcessor {
	private static final BizLog bizlog = BizLogUtil.getBizLog(ApStrikeEvntProcessorCD03.class);

	@Override
	public void process(E_STACPS stacps, E_COLOUR colour, KnbEvnt evnt) {
		if (CommUtil.compare(evnt.getTranev(), ApUtil.TRANS_EVENT_CD_POST) != 0)
			throw ApError.Aplt.E0000("交易事件不是[" + ApUtil.TRANS_EVENT_CD_POST + "]，不能冲正！");
			...
			SysUtil.getInstance(IoCdHouseHoldSvcType.class).postVirtAcctStrike(cplAccountIN);

			bizlog.debug("对公账户一户通虚户存入冲正处理结束=====================");
	}
