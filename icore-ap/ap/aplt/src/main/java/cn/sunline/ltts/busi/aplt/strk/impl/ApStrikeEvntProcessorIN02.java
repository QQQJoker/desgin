...
public class ApStrikeEvntProcessorIN02 implements ApStrikeEvntProcessor {
	private static final BizLog bizlog = BizLogUtil.getBizLog(ApStrikeEvntProcessorIN02.class);

	@Override
	public void process(E_STACPS stacps, E_COLOUR colour, KnbEvnt evnt) {
		if (bizlog.isDebugEnabled()) {
			bizlog.debug("内部户借方交易冲正处理开始=====================[%s]",evnt);
		}
		
		if (CommUtil.compare(evnt.getTranev(), ApUtil.TRANS_EVENT_INACDR) != 0) {
			throw ApError.Aplt.E0000("交易事件不是[" + ApUtil.TRANS_EVENT_INACDR + "]，不能冲正！");
		}
		...
		if (SysUtil.getInstance(IoHotCtrlSvcType.class).selHcpDefn(evnt.getTranac()) == E_YES___.YES) {
			...
			info.setCorrtg(cn.sunline.ltts.busi.sys.type.InEnumType.E_CORRTG._1);
			...
			info.setAmntcd(E_AMNTCD.DR);
			info.setStrktg(E_YES___.NO);
			info.setSttssq(evnt.getTransq());// 交易流水
			info.setDscrtx(CommToolsAplt.prcRunEnvs().getRemark());// 冲账备注登记

			// 调用借方服务
			SysUtil.getInstance(IoInAccount.class).ioInAcdr(info);
		}
		if (bizlog.isDebugEnabled()) {
			bizlog.debug(" 内部户借方交易冲正处理结束=====================");
		}
	}
