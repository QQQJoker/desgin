...
public class ApStrikeEvntProcessorIN04 implements ApStrikeEvntProcessor {
	private static final BizLog bizlog = BizLogUtil.getBizLog(ApStrikeEvntProcessorIN04.class);

	@Override
	public void process(E_STACPS stacps, E_COLOUR colour, KnbEvnt evnt) {
		if (bizlog.isDebugEnabled()) {
			bizlog.debug("内部户收方交易冲正处理开始=====================[%s]", evnt);
		}

		if (CommUtil.compare(evnt.getTranev(), ApUtil.TRANS_EVENT_INACRV) != 0) {
			throw ApError.Aplt.E0000("交易事件不是[" + ApUtil.TRANS_EVENT_INACRV + "]，不能冲正！");
		}
		...
		if (SysUtil.getInstance(IoHotCtrlSvcType.class).selHcpDefn(evnt.getTranac()) == E_YES___.YES) {
			...
			info.setAcbrch(CommToolsAplt.prcRunEnvs().getTranbr());

			// 调用借方服务
			SysUtil.getInstance(IoInAccount.class).ioInAcrv(info);
		}
		if (bizlog.isDebugEnabled()) {
			bizlog.debug("内部户收方方交易冲正处理结束=====================");
		}
	}
