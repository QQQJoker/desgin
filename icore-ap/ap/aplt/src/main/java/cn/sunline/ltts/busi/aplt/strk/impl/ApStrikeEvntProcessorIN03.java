...
public class ApStrikeEvntProcessorIN03 implements ApStrikeEvntProcessor {
	private static final BizLog bizlog = BizLogUtil.getBizLog(ApStrikeEvntProcessorIN03.class);

	@Override
	public void process(E_STACPS stacps, E_COLOUR colour, KnbEvnt evnt) {
		if (bizlog.isDebugEnabled()) {
			bizlog.debug("内部户付方冲正处理开始=====================[%s]",evnt);
		}

		if (CommUtil.compare(evnt.getTranev(), ApUtil.TRANS_EVENT_INACPV) != 0) {
			throw ApError.Aplt.E0000("交易事件不是[" + ApUtil.TRANS_EVENT_INACPV + "]，不能冲正！");
		}
		...
		if (SysUtil.getInstance(IoHotCtrlSvcType.class).selHcpDefn(evnt.getTranac()) == E_YES___.YES) {
			...
			info.setAmntcd(E_AMNTCD.PY);
			info.setSttssq(evnt.getTransq());// 交易流水
			info.setDscrtx(CommToolsAplt.prcRunEnvs().getRemark());// 冲账备注登记

			// 调用贷方服务
			SysUtil.getInstance(IoInAccount.class).ioInAcpv(info);
		}
		if (bizlog.isDebugEnabled()) {
			bizlog.debug("内部户付方交易冲正处理结束=====================");
		}
	}
