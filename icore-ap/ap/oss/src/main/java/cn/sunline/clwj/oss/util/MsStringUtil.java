...
public class MsStringUtil {
    ...
    public static String nullable(Object obj, String defaultValue) {
        return MsStringUtil.isEmpty(obj) ? defaultValue : obj.toString();
    }
     ...
     * 
     * @see MsStringUtil#escapeForXML(String)
     * @param input
     *        待反编码字符串
     * @return 反编码后的字符串.
     */
    ...
    public static String maxstr(String s, String encoding, int maxlength) {
        ...
        try {
            ...
            //2016.3.22 BUG-FIX 修复在部分字符集的情况下，截取的长度超过指定的最大长度的问题
    ...
    /**
     * 根据当前数据库字符集获取带汉字字串的length 将带字符串按Byte位长度取子字符串 防止带汉字的字符串长度取错
     * 2015.5.7
     * @param String 源字符串
     * @param String 数据库字符集 
     * @return int Byte位长度
	...
	private static final char DELIM_STOP = 's';


	public static void format(StringBuilder destSB,
		...
		for (int i = 0; i < messagePattern.length(); i++) {
			...
			if (DELIM_START == c && messagePattern.charAt(i + 1) == DELIM_STOP) {
				destSB.append(String.valueOf(arguments[argsIndex]));
				argsIndex++;
				i++;
			}else if (DELIM_START == c && messagePattern.charAt(i + 1) != DELIM_STOP) {//兼容其他方式，如：%f 
				destSB.setLength(0);//清空，按原来的方式处理
