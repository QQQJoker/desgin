 ...
 * 
 */
public class SMSBatchWaitSendUtil extends LttsTimerProcessor {
    ...
    public void process(String param, DataArea dataArea) {
        ...
        // 查询出有效的非实时的未处理短息集合
        List<ApAmsg.ApsShorUndo> list = ApMsgNsqlDao.selWaitApsShorundoByGroupId(minGroupId, maxGroupId, dataArea.getCommReq().getString("corpno"), false);
        // 采用批量定时短信发送接口实现发送短信
        ...
        for (final ApAmsg.ApsShorUndo apsShorundo : list) {
            ...
            try {
                ...
                //TODO 对接日志监控平台
                KnpPara knpPara = CommTools.KnpParaQryByCorpno("BatchPublishSMSGroup", "retrycount", "%", "%", true);
                log.error("###发送[%s]短信失败###现进行重试机制，重试[%s]次", e, CommTools.toJson(sMSCType), knpPara.getPmval1());
                for (int i = 0; i < Integer.valueOf(knpPara.getPmval1()); i++) {
                    try {
                        smsSender.sendSMSes(smsMessageList);
                        log.error("##重试第[%s]次后，发送[%s]短信成功", i, CommTools.toJson(sMSCType));
                        break;
                    } catch (Exception e1) {
                        log.error("##重试第[%s]次后，发送[%s]短信然后失败###继续重试", e1, i, CommTools.toJson(sMSCType));
                    }
                }
            } finally {
