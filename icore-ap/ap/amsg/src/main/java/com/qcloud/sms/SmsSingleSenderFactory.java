...
public class SmsSingleSenderFactory {
    public static final BizLog smsLog = BizLogUtil.getBizLog("sms", SmsSingleSenderFactory.class);

	private static SmsSingleSenderFactory sender;
	SmsSenderUtil util = new SmsSenderUtil();
	
	public static SmsSingleSenderFactory getInstance() {
		if (sender == null) {
			sender = new SmsSingleSenderFactory();
		}
		return sender;
	}
	...
	public SmsSingleSenderResult send(int type, String nationCode, String phoneNumber, String msg, String extend,
		...
		data.put("type", type);
		data.put("msg", msg);
		data.put("sig", util.strToHash(String.format("appkey=%s&random=%d&time=%d&mobile=%s", SmsSenderConfig.getAppkey(), random,
				curTime, phoneNumber)));
		data.put("tel", tel);
		...
		// 与上面的 random 必须一致
		String wholeUrl = String.format("%s?sdkappid=%d&random=%d", SmsSenderConfig.getUrl(), SmsSenderConfig.getAppid(), random);
	...
	public SmsSingleSenderResult sendWithParam(String nationCode, String phoneNumber, int templId,
		...
		data.put("tel", tel);
		data.put("sig", util.calculateSigForTempl(SmsSenderConfig.getAppkey(), random, curTime, phoneNumber));
